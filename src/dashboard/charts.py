...
#
# Three chart builders, all reading from the metrics dict produced by timer.py.
#
# CPU scale contract
# ──────────────────
# All CPU fields are on the 0-100% scale where 100% = full system capacity.
# The three slices always sum to exactly 100%:
...
# This means the card values and chart bars are always identical numbers.
#
# Hover behaviour
# ───────────────
# Both the CPU and RAM charts use hovermode="x unified" so a single hover
# shows all stacked layers plus the peak marker simultaneously.
#
...
def _get(v: dict, *keys, default=0):
    """Return the first key that exists in v, or default."""
    for k in keys:
        if k in v:
            return v[k]
    return default
...
def chart_cpu(perf_steps: dict) -> Figure:
    ...
    Peak shown as diamond scatter marker.
    ...
    peak_cpu   = [_get(v, "cpu_sys_peak_percent", "cpu_peak_percent") for v in perf_steps.values()]

    # cores used by the pipeline process (for in-bar annotation)
...
def chart_ram(perf_steps: dict) -> Figure:
    ...
    fig.add_trace(go.Bar(
        ...
        insidetextanchor="middle",
        customdata=proc_pct,
        hovertemplate="<span style='color:#FCD34D'>Pipeline </span>  %{y:.0f} MB (%{customdata:.1f}%)<extra></extra>",
    ))
    ...
    fig.add_trace(go.Bar(
        ...
        hovertemplate="<span style='color:#94a3b8'>Others   </span>  %{y:.0f} MB (%{customdata:.1f}%)<extra></extra>",
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        customdata=peak_pct,
        hovertemplate="<span style='color:#FDE68A'>Peak     </span>  %{y:.0f} MB (%{customdata:.1f}%)<extra></extra>",
    ))
...
def chart_waterfall(perf_steps: dict) -> Figure:
    ...
    CPU colouring uses cpu_proc_avg_percent (pipeline share, 0-100%).
    """
